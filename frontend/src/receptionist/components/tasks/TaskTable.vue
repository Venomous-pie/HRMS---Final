      ...
      <tbody>
        ...
        <template v-else v-for="category in taskCategories" :key="category.name">
          ...
          <template v-if="expandedCategories[category.type]">
            <tr v-for="item in category.items" :key="item.id" class="border-t border-gray-100"
              :ref="(el) => setRowRef(item.id, el)">
              <TaskRow :item="item" :category="category" :time-slots="timeSlots" :hovered-column="hoveredColumn"
                @column-hover="$emit('columnHover', $event)" @column-leave="$emit('columnLeave')"
                @cell-click="$emit('cellClick', $event)" @item-click="() => { }" />
            </tr>
          </template>
        </template>
      </tbody>
        ...
        <template v-if="expandedCategories[category.type]">
          <template v-for="item in category.items" :key="item.id">
            <template v-for="task in item.tasks" :key="task.id">
              <TaskSpan v-if="getTaskSpanStyle(task, item.id, category)" :task="task" :category-color="category.color"
...
import { TIME_GANTT_LAYOUT, findTimeRangeIndices, calculateTaskSpanStyle, assignTaskLayers } from '@/utils/timeGantt'
...
// Compute task layers for each item to prevent overlap
const taskLayers = computed(() => {
  const layersMap = new Map<string, Map<string, number>>() // itemId -> taskId -> layer

  for (const category of props.taskCategories) {
    if (!props.expandedCategories[category.type]) continue

    for (const item of category.items) {
      if (!item.tasks || item.tasks.length === 0) continue

      // Assign layers to tasks in this item
      const layerMap = assignTaskLayers(item.tasks)
      layersMap.set(item.id, layerMap)
    }
  }
...
const getTaskSpanStyle = (task: any, itemId: string, category: any): any | null => {
  ...
  // Get the layer for this task (0 if not found, which means no overlap)
  const itemLayers = taskLayers.value.get(itemId)
  const layer = itemLayers?.get(task.id) ?? 0

  return calculateTaskSpanStyle(indices.startIndex, indices.endIndex, rowTop, layer)
}
